...
from ltr_db_optimizer.model.model_interface_lero import create_model
import pickle
import os
...
parser.add_argument("--loss_func", type=str, default="ranknet")
...
with open(f"{root_path}/training_data/lero_{score_func}_{training_workload}{postfix}_query_enc_norm.pickle", "rb") as f:
    vectors = pickle.load(f)
with open(f"{root_path}/training_data/lero_{score_func}_{training_workload}{postfix}_plan_enc_norm.pickle", "rb") as f:
    plans = pickle.load(f)
with open(f"{root_path}/training_data/lero_{score_func}_{training_workload}{postfix}_labels_norm.pickle", "rb") as f:
...
total_jobs = list(set(list(map(lambda x:x.split("_")[0], list(plans.keys())))))


split_valid = random.sample(total_jobs, int(0.2*len(total_jobs)))
...
for key in sorted_plans.keys():
    ...
    elif key.split("_")[0] in split_valid:
        X_valid_tree[key] = {}
        X_valid_vecs[key] = {}
        for subkey in sorted_plans[key].keys():
...
bash train_and_enum_VM_lero.sh linearR lero lero stats-r stats 347thSamTrain100%FixValid20%SamSize20NoScheduler stats1000reshuff 6Q10D98pTree17591100%1 &


bash train_and_enum_VM_lero.sh linearR lero lero imdb-r imdb 348thSamTrain100%FixValid20%SamSize20NoScheduler tpch1000 Q6D10MaxTree28796000000000%1 &


bash train_and_enum_VM_lero.sh linearR lero lero job-c imdb 356thSamTrain100%FixValid20%SamSize20NoScheduler imdb39reshuff  Q6D18MaxTree1547830000%1 &


bash train_and_enum_VM_lero.sh linearR lero lero imdb-r imdb 359thSamTrain100%FixValid20%SamSize20NoScheduler imdb39reshuff Q6D31MaxTree1547830000%1  &
