...
def get_the_split_of_training_data_finetune(plan_keys, valid_ratio, test_ratio, workload):
    '''
    ...
    print('the number of subqueries for validation: ', len(split_valid))
    print('the number of subqueries for test: ', len(split_test))
    print('the number of subqueries for training: ', len([job for job in key_df["Subquery"].values.tolist() if job not in (split_valid+split_test)]))

    split_train = [job for job in key_df["Subquery"].values.tolist() if job not in (split_valid+split_test)]
    return split_valid, split_test, split_train
...
def add_force_scan_to_sql_without_hint(sql):
    # input_sql = sql.upper()
    ...
    # sql_with_hint = sql_with_hint + f" OPTION (MAXDOP 1, USE HINT ('QUERY_OPTIMIZER_COMPATIBILITY_LEVEL_140'), {hint} ) "


    sql_with_hint = sql_with_hint + f" OPTION (MAXDOP 1, USE HINT ('QUERY_OPTIMIZER_COMPATIBILITY_LEVEL_140', 'FORCE_DEFAULT_CARDINALITY_ESTIMATION')) "
...
def add_force_scan_to_sql(sql, hint=""):
    # input_sql = sql.upper()
    print('initial sql: ', sql)
    input_sql = copy.copy(sql)
    before_from, after_from = input_sql.split('FROM')
    ...
    sql_with_hint = sql_with_hint + f" OPTION (MAXDOP 1, USE HINT ('QUERY_OPTIMIZER_COMPATIBILITY_LEVEL_140', 'FORCE_DEFAULT_CARDINALITY_ESTIMATION'), {hint} ) "

    # sql_with_hint = sql_with_hint + f" OPTION (MAXDOP 1, {hint}, USE HINT ('FORCE_DEFAULT_CARDINALITY_ESTIMATION')) "
...
def enumerate_by_SQL_Server_FAST(job, sql_full, folder_name, cursor, iter, hint=""):
    ...
    sql_full = add_force_scan_to_sql(sql_full, hint=hint)

    rows = cursor.execute(sql_full).fetchall()

    enum_end_time = datetime.now()
    enum_delta_time = (enum_end_time - start_time).total_seconds()*1000
    ...
    # sql_full_without_addtional_string = sql_full.replace(f" OPTION (MAXDOP 1, USE HINT ('QUERY_OPTIMIZER_COMPATIBILITY_LEVEL_140', 'FORCE_DEFAULT_CARDINALITY_ESTIMATION'), {hint}) ", "")

    # sql_full_without_addtional_string = sql_full.replace(f" OPTION (MAXDOP 1, {hint}, USE HINT ('FORCE_DEFAULT_CARDINALITY_ESTIMATION')) ", "")



    # sql_full_without_addtional_string = sql_full.replace(f" OPTION (MAXDOP 1, USE HINT ('QUERY_OPTIMIZER_COMPATIBILITY_LEVEL_140'), {hint}) ", "")


    # sql_full_without_addtional_string = sql_full.replace(f" OPTION (MAXDOP 1, {hint} ) ", "")
    ...
    return enum_delta_time

def enumerate_by_SQL_Server_FAST_without_hint(job, sql_full, folder_name, cursor, iter):
    ...
    sql_full = add_force_scan_to_sql_without_hint(sql_full)
    ...
    sql_full_without_addtional_string = sql_full.replace(f" OPTION (MAXDOP 1, USE HINT ('QUERY_OPTIMIZER_COMPATIBILITY_LEVEL_140', 'FORCE_DEFAULT_CARDINALITY_ESTIMATION')) ", "")


    # sql_full_without_addtional_string = sql_full.replace(f" OPTION (MAXDOP 1, {hint} ) ", "")


    # sql_full_without_addtional_string = sql_full.replace(f" OPTION (MAXDOP 1, USE HINT ('QUERY_OPTIMIZER_COMPATIBILITY_LEVEL_140', 'FORCE_LEGACY_CARDINALITY_ESTIMATION'), {hint} ) ", "")
