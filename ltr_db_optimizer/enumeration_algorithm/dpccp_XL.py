...
class DPccp:
    ...
    def enumerate(self, name_in_data=False):
        ...
        for csg, cmp in self.get_csg_cmp_pairs():
            ...
            if len(possible_joins) > self.top_k:
                ###prune the new csg-cmp pair candidates
                print('prune the new candidates')
                print('full_name', full_name, len(possible_joins))
                possible_joins = self.reduce(possible_joins)
                print('after prune:', len(possible_joins))
    ...
    def enumerate_XL(self, name_in_data=False):
        """
        :param graph: networkx Graph in BFS
        """
        best_parts = {}
        ...
        for csg, cmp in self.get_csg_cmp_pairs():
            ...
            if csg_name in best_parts:
                # print('inner#csg:', csg_name, len(best_parts[csg_name]), best_parts[csg_name])
                # if len(best_parts[csg_name]) > self.top_k:
                #     best_parts[csg_name] = self.reduce(best_parts[csg_name])
                right = best_parts[csg_name]
            if cmp_name in best_parts:
                # print('inner#cmp:', cmp_name, len(best_parts[cmp_name]), best_parts[cmp_name])
                # if len(best_parts[cmp_name]) > self.top_k:
                #     best_parts[cmp_name] = self.reduce(best_parts[cmp_name])
            ...
            # full = copy.copy(full_name)
            print('accumulated full:', len(best_parts[full_name]))
            if len(best_parts[full_name]) > self.top_k:
                print('ccc', full_name, len(best_parts[full_name]))

                best_parts[full_name] = self.reduce(best_parts[full_name])
                print('fff', full_name, len(best_parts[full_name]))
                # return self.reduce(best_parts[full])#[0]
            # else:
            #     print('ddd')
        print('final full name', full_name, len(best_parts[full_name]))

        return best_parts[full_name]
