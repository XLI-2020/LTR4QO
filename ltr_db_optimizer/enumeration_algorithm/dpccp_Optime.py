...
class DPccp:
    ...
    def reduce_adaptK(self, plans, last = False):
        ...
        if self.model is None:
            ...
            nr_of_candidates = len(predictions)
            if nr_of_candidates <= self.top_k*10:
                k = self.top_k if not last else 1
            else:
                k = int(nr_of_candidates*0.1) if not last else 1
    ...
    def reduce(self, plans, last = False):
        ...
        if self.model is None:
            ...
            enum_time_inference = round(enum_time_inference, 2)
