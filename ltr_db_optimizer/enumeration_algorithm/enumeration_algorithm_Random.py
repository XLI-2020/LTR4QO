...
from ltr_db_optimizer.enumeration_algorithm.dpccp_random import DPccp
...
class EnumerationAlgorithm(DPccp):
    def __init__(self, sql_query, table_info, model, sql_text, top_k = 1, job_name=None, nr_workload=None, alias_dict = {}, comparison=False):
        self.path = model
        super().__init__(model = model, job_name=job_name, nr_workload=nr_workload, joiner = Joiner(sql_query, alias_dict, table_info), comparison=comparison)
        self.top_k = top_k
    ...
    def find_best_plan(self):
        ...
        if len(best_plan) > 1:
            best_plan = self.reduce('last', best_plan, True)
