...
from ltr_db_optimizer.enumeration_algorithm.dpccp_lero import DPccp
...
from ltr_db_optimizer.model.featurizer_graph_lero import FeatureExtractorGraph, long_query_encode
...
class EnumerationAlgorithm(DPccp):
    def __init__(self, sql_query, table_info, model, sql_text, top_k = 1, alias_dict = {}, train_wk=None, tree_high=None, tree_low=None, comparison=False):
        self.path = model
        super().__init__(model = model, table_info=table_info, joiner = Joiner(sql_query, alias_dict, table_info), comparison=comparison)
        self.top_k = top_k
    ...
    def plans_to_feature_vecs(self, sql, plans, last):
        ...
        for it, plan in enumerate(plans):
            ...
            featurize_st_time = datetime.now()

            temp, rows = self.featurizer.featurize_node(plan)
            featurize_end_time = datetime.now()
            temp_featurize_time = (featurize_end_time - featurize_st_time).total_seconds() * 1000
            # print('temp:', len(temp), temp)
            result_featurized.append(temp)
            self.total_featurize_time += temp_featurize_time
            self.total_featurize_time = round(self.total_featurize_time, 2)
