...
from ltr_db_optimizer.enumeration_algorithm.dpccp_COST import DPccp
...
class EnumerationAlgorithm(DPccp):
    ...
    def prepare_plans(self, plans, last = False):
        ...
        plans_costs = self.plans_to_feature_vecs(sql, plans, last)
        query_enc = plans[0].get_query_encoding()
        return query_enc, plans_costs
    ...
    def plans_to_feature_vecs(self, sql, plans, last):
        ...
        for it, plan in enumerate(plans):
            try:
                ...
                # plan_sql = sql + " OPTION (RECOMPILE, USE HINT ('FORCE_DEFAULT_CARDINALITY_ESTIMATION'), USE PLAN N'" + xml + "')"

                # plan_sql = sql + " OPTION (RECOMPILE, MAXDOP 1,  USE HINT ('FORCE_DEFAULT_CARDINALITY_ESTIMATION'), USE PLAN N'" + xml + "')"

                # plan_sql = sql + " OPTION (MAXDOP 1, USE HINT ('QUERY_OPTIMIZER_COMPATIBILITY_LEVEL_140', 'FORCE_DEFAULT_CARDINALITY_ESTIMATION')) "
                ...
                print('{it} Exception:'.format(it=it), exce)
                continue
