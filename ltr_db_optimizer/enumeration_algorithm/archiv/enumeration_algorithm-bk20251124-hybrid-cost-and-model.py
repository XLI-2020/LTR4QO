...
class EnumerationAlgorithm(DPccp):
    ...
    def prepare_plans_long_queryenc(self, plans, last = False):
        if last:
            sql = self.sql_text
        else:
        ...
        query_enc = [long_query_enc + plan.get_query_encoding() for plan in plans]

        return query_enc, feat_plans

    def prepare_plans_original(self, plans, last = False):
        if last:
            sql = self.sql_text
        else:
            sql = to_sql(plans[0], self.table_info)
    ...
    def prune_by_Cost_first(self, sql, plans, prun_thre):
        ...
        for it, plan in enumerate(plans):
            ...
            plans_costs.append((plan, estimated_cost))

        conn.close()

        sorted_plans_costs = sorted(plans_costs, key=lambda x: x[1], reverse=False)

        print('sorted_plans_costs: ', len(sorted_plans_costs), list(sorted_plans_costs)[:10])

        selected_plans = [item[0] for item in list(sorted_plans_costs)[:prun_thre]]
    ...
    def prepare_plans(self, plans, last=False):
        ...
        # prune plans by Cost first

        plans_filtered_by_Cost = self.prune_by_Cost_first(sql, plans, prun_thre=20)

        plans = copy.copy(plans_filtered_by_Cost)
    ...
    def plans_to_feature_vecs_Cost_based(self, sql, plans, last):
        parser = XMLParser(table_info=self.table_info, small_version=True)
        rows = 0
        result_featurized = []
