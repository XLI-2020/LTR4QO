...
class DPccp:
    ...
    def reduce(self, plans, last = False):
        ...
        if self.model is None:
            ...
            p_sort = torch.sort(predictions_flat, descending = True)
            p_sort_diff = torch.diff(p_sort.values)
            till_index = p_sort_diff.argmin()
            if till_index > 50:
                print('till_index over bound!!')
                till_index = 50
            selected_indices = p_sort.indices[:till_index+1]
