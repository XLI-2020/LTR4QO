...
class DPccp:
    ...
    def __init__(self, model = None, graph = None, joiner = None, top_k = 1, comparison = False):
        ...
        if model:
            pretained_model = model.split("_FT")[0]
            pretrained_model_pth = f"/home/xliq/Documents/LTR_DP/ltr_db_optimizer/model/saved_models/{pretained_model}/min_avg_valid_loss.pth"
            if not comparison:
                ...
                self.model.load_state_dict(torch.load(pretrained_model_pth))
            else:
        ...
        output_net_pth = f"/home/xliq/Documents/LTR_DP/ltr_db_optimizer/model/saved_models/{model}/min_avg_valid_loss.pth"
        self.output_net.load_state_dict(torch.load(output_net_pth))
    ...
    def reduce_original(self, plans, last = False):
    ...
    def reduce(self, plans, last = False):
        ...
        if self.model is None:
            ...
            # query_enc = torch.tensor(query_enc, dtype=torch.float).to(self.device)
            # print('query encocccc222:', query_enc.dtype)
            extracted_feature = self.model.feature_extract(query_enc, feat_plans)

            print("extracted_features", extracted_feature.shape)

            predictions = self.output_net.online_predict_all(extracted_feature).t()

            print("predictions haha FT", predictions.shape, predictions)


            bound = min(len(predictions[0]), 50)
            print('bound', bound, len(predictions[0]))
            topk_predictions = torch.topk(predictions[0], bound, dim=-1)
            print('topk predictions: ', topk_predictions)

            p_sort_diff = torch.diff(topk_predictions.values)
            till_index = p_sort_diff.argmin()
            print('till_index', till_index)

            selected_indices = topk_predictions.indices[:till_index+1]
            print('selected_indices', selected_indices)

            selected_plans = [plans[i] for i in selected_indices.tolist()]
