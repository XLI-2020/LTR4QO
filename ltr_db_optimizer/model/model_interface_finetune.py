...
def create_model(LossFunction, **kwargs):
    class ModelInterface(LossFunction):
        ...
        def __init__(self, epochs=50, sample_size=20, batch_size=64, workload=None, name="", output_net_name="", folder="", **kwargs):
            ...
            if model_archi_name == "HM":
                self.pretrain_net = LTRComparisonNet(input_dim_1, input_dim_2).to(self.device)
                print('load LTRComparisonNet HM!!!')
                # self.net_ndcg = LTRComparisonNet(input_dim_1, input_dim_2).to(self.device)
            else:
                self.pretrain_net = eval(model_archi_name)(input_dim_1, input_dim_2).to(self.device)
                print(f'load XL proposed model:{model_archi_name}!!!')
                print('load LTRComparisonNet XL!!!')

            pretrained_model_pth = f"/home/xliq/Documents/LTR_DP/ltr_db_optimizer/model/saved_models/{self.name}/min_avg_valid_loss.pth"

            self.pretrain_net.load_state_dict(torch.load(pretrained_model_pth))
            self.net = OutputNet().to(self.device)

            self.output_name = output_net_name

            print('output model saving name: ', self.output_name)

            if os.path.exists(f"{folder}/{self.output_name}"):
                print('Model file already exists and needs to be deleted first!!!')
                os.system(f'rm -rf {folder}/{self.output_name}')
                print('after deleting, check if the model file still exists: ',
                      os.path.exists(f"{folder}/{self.output_name}"))
            else:
        ...
        def fit(self, X_train_vecs, X_train_tree, y_train, X_valid_vecs, X_valid_trees, y_valid,  use_presort=False,
            ...
            if not os.path.exists(self.folder + "/" + self.output_name):
                os.makedirs(self.folder + "/" + self.output_name)
            ...
            for epoch in range(self.epochs):
                ...
                # for curr_sample_size in sample_size_list:
                    ...
                    pretrain_features = self.pretrain_net.feature_extract_batch(batch_x_q, batch_x_p, plan_num=curr_sample_size)
                    pred_results = self.net(pretrain_features)
                ...
                with torch.no_grad():
                    ...
                    for valid_batch_i in range(num_valid_batches):
                        ...
                        y_pretrain_feats = self.pretrain_net.feature_extract_batch(valid_batch_x_q, valid_batch_x_p, plan_num=self.sample_size)
                        y_predicted = self.net.predict_all(y_pretrain_feats)
                ...
                if avg_valid_ndcg > best_ndcg:
                    ...
                    torch.save(self.net.state_dict(), f"{self.folder}/{self.output_name}/best_avg_ndcg.pth")

                if avg_valid_loss < min_valid_loss:
                    print("save model because of valid loss!")
                    min_valid_loss = avg_valid_loss
                    best_validloss_epoch = epoch
                    torch.save(self.net.state_dict(), f"{self.folder}/{self.output_name}/min_avg_valid_loss.pth")
            ...
            with open(self.folder + "/" + self.output_name + "/train_and_valid_info.pickle", "wb") as f:
