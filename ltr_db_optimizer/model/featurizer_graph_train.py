...
def featurize_with_labels_Random(plans_folder, cost_folder, label_csv, workload="tpch", nr_jobs=1000, max_score = 50, score_function = "special",  extra_for_min = True, special_border = 0.95, postfix=None):
    ...
    #test only with whole plans/queries
    # print('before whole:', len(df))
    # df = df[df.index.isin(plans_with_last)]
    # print('after whole:', len(df))
    #test only with whole plans/queries


    # print('before good:', len(df))
    #test only with the best plans as training data
    # groupby_sq = df.groupby("Subquery")
    #
    # good_plans_list = []
    # topk_good = 10
    #
    # for g_name, group in groupby_sq:
    #     group_df = pd.DataFrame(group)
    #     group_df = group_df.sort_values(by = ["cost"], ascending = True)
    #     good_plan_df = group_df.iloc[:topk_good]
    #     good_plans_list.append(good_plan_df)
    #
    # df = pd.concat(good_plans_list, axis = 0)
    #test only with the best plans as training data
    # print('after good:', len(df))
    ...
    if score_function == "special":
        df = df.head(80000) ### for HM because it uses cluster-based labels
    ...
    if score_function == "special":
        print('times', len(times), times[:5])
        print('max_score', max_score)
        print('special_border', special_border)
