...
class FeatureExtractorGraph:
    ...
    def __init__(self, with_cost = True, with_issort = False, workload = 'tpch'):
        ...
        if 'tpch' in workload or "Db" in workload:
            ...
            self.tree_high = 53435600 #28796000000000.0 #53435600.0 #4.801740e+08 #53435600.0
            self.tree_low = 1.0
            ...
            self.tree_high = 53435600 # 53435600.0 # 4.801740e+08
            self.tree_low = 1.0
            ...
            self.tree_high = 7.3#18349800.0 #95000  #
            self.tree_low = 0.3
    ...
    def append_features(self, execution_plan, label_parts):
        ...
        if not(execution_plan.name == "top" and execution_plan.get_left_child().name == "sort"):
            ...
            if not execution_plan.has_rows():
                ...
                log_rows = np.log10(estimated_rows + 1)

                rows_calc = (estimated_rows-self.tree_low)/(self.tree_high-self.tree_low)

                # rows_calc = (log_rows-self.tree_low)/(self.tree_high-self.tree_low)
                ...
                if log_rows > self.max_tree_high:
                    self.max_tree_high = log_rows
                    print('max_tree_high: ', self.max_tree_high)
                if log_rows < self.min_tree_high:
                    self.min_tree_high = log_rows
                    print('min_tree_high: ', self.min_tree_high)
                # self.tree_len_list.append(estimated_rows)
                # tree_mean = np.mean(self.tree_len_list)
                # tree_std = np.std(self.tree_len_list)
                # print('tree_mean, tree_std ', tree_mean, tree_std)
...
def get_features_with_cost_from_folder_Random(plans_folder, cost_folder, workload='tpch', plans_list=None, return_featurized=True):
    # feature_ext = FeatureExtractor()
    feature_ext = FeatureExtractorGraph(workload=workload)
    featurized_trees = {}
    featurized_vecs = {}
    ...
    for job in jobs_list:
        ...
        with open(f"{cost_folder}/{job}/last/sp0.txt", "r") as f:
            plan_xml = f.read()
...
def featurize_with_labels_Random(plans_folder, cost_folder, label_csv, workload="tpch", nr_jobs=2000, max_score = 50, score_function = "special", extra_for_min = True, special_border = 0.95):
    ...
    featurized_vecs, featurized_trees = get_features_with_cost_from_folder_Random(plans_folder, cost_folder, workload=workload, plans_list=involved_plans_list)
