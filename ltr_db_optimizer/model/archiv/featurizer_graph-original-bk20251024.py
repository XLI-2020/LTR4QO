...
class FeatureExtractorGraph:
    ...
    #         # print('execution_plan name outside: ', execution_plan.name)
    ...
    #                 print('estimated number of rows: ', curr_part[1])
    ...
    #                 # print('execution_plan.right_child', execution_plan.right_child)
    ...
    for file in os.listdir(cost_folder):
        if file.endswith(".txt"):
            ...
            if return_featurized:
                ...
                print('type of execution_plan node: ', execution_plan)
                query_vec = execution_plan.get_query_encoding()
                print('query_encoding: ', query_vec)
...
# def featurize_with_labels(plans_folder, cost_folder, label_csv, max_score=50, score_function="special",
#                           extra_for_min=True, special_border=0.95):
#     # featurized_vecs, featurized_trees = get_features_with_cost_from_folder(plans_folder, cost_folder)
#
#     # print('featurized_vecs', featurized_vecs, featurized_trees)
#     label_dict = {}
...
#     times = []
#
#     print('score function: ', score_function)
#
#     for job in pd.unique(df["Job_nr"]):
#         temp_df = df[df["Job_nr"] == job].copy()
#         print('job: ', job)
...
#         a = np.array(temp_df["Sum"])
#
#         print('temp_df["CPU time"]: ', temp_df["CPU time"])
#
#         print('temp_df["Sum"]: ', temp_df["Sum"])
#
#         if score_function == "special":
#             temp = df[df["Job_nr"] == job]
#             labels = temp.index
...
#                 a[a == -2] = max(a) * 2  # Necessary, otherwise min(a) == -2
#                 temp_df["scores"] = calculate_linear_scores(a, n=max_score)
#                 print('Linear func:',  temp_df["scores"])
#
#             elif score_function == "histogram":
#                 temp_df["scores"] = calculate_histogram_score(a, nr_bins=max_score, extra_bin_for_min=extra_for_min)
#             elif score_function == "agglomerative":
#                 temp_df["scores"] = calculate_agglomerative_score(a, nr_clusters=max_score,
#                                                                   extra_bin_for_min=extra_for_min)
#
#             for idx, row in temp_df.iterrows():
#                 label_dict[idx] = row["scores"]
#
...
#         labels, scores = calculate_special_score(times, max_score, special_border)
#
#         print('Special func:', labels, scores)
#         for idx, s in enumerate(scores):
#             label_dict[labels[idx]] = s
#
#     return label_dict, 1, 2
#
