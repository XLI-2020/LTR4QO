...
# Job
min_vec = np.array([0.0, 0.0, 0.0, 1.0, 4.0, 4.0])
max_vec = np.array([1.0, 1.0, 23.0, 4.801740e+08, 36244343, 36244343])
tree_high = 4.801740e+08
tree_low = 1.0
...
class FeatureExtractorGraph:
    ...
    def __init__(self, with_cost = True, small_version=False, with_issort = True):
    ...
    def featurize_node(self, node):
        ...
        if node.has_right_child():
            ...
            # print('left_child node name & id: ', left_child.name, left_child.id, left_child.estimated_rows)
            # print('right_child node name & id: ', right_child.name, right_child.id, right_child.estimated_rows)


            if self.with_issort:
                if isinstance(left_child, nodes.SortNode):
                    this_vector[-2] = 1
                if isinstance(right_child, nodes.SortNode):
                    this_vector[-3] = 1
...
def  get_features_with_cost_from_folder(plans_folder, cost_folder, plans_list=None, return_featurized=True):
    ...
    for plan_nr in plans_list:
        ...
        version_nr = plan_nr.split("_")[1]
        cost_file_name = plan_nr + ".txt"

        with open(cost_folder+"/"+cost_file_name, "r") as f:
            cost_plan = f.read()
...
def featurize_with_labels(plans_folder, cost_folder, label_csv, nr_jobs=2000, max_score = 50, score_function = "special", extra_for_min = True, special_border = 0.95):
    ...
    featurized_vecs, featurized_trees = get_features_with_cost_from_folder(plans_folder, cost_folder, plans_list=involved_plans_list)
...
def get_features_with_cost_from_folder_Random(plans_folder, cost_folder, plans_list=None, return_featurized=True):
    # feature_ext = FeatureExtractor()
    feature_ext = FeatureExtractorGraph()
    featurized_trees = {}
    featurized_vecs = {}
    ...
    #     long_query_enc = long_query_encode(sql_full)
    ...
    for plan_nr in plans_list:
        ...
        if return_featurized:
            ...
            # total_query_enc = query_enc_dict[job_nr] + query_vec

            total_query_enc = query_vec


            featurized_trees[plan_nr] = featurized_plan
            featurized_vecs[plan_nr] = total_query_enc
        else:
            featurized_trees[plan_nr] = full_execution_plan
    return featurized_vecs, featurized_trees


def featurize_with_labels_Random(plans_folder, cost_folder, label_csv, nr_jobs=2000, max_score = 50, score_function = "special", extra_for_min = True, special_border = 0.95):
    ...
    featurized_vecs, featurized_trees = get_features_with_cost_from_folder_Random(plans_folder, cost_folder, plans_list=involved_plans_list)
