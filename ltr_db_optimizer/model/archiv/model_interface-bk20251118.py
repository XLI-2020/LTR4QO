...
def get_the_split_of_jobs_list(X_train_vecs, train_ratio):
    '''
    group training data by the distribution of runtime or the number of joins
    '''
    # seed = 139
    ...
    for subquery in subquery_list:
        ...
        db = 'job'
        if db == "tpch":
            with open(f"/home/xliq/Documents/LTR_DP/Data/subplans/{job_nr}/{subquery_nr}/{plan_nr}.txt", "r") as f:
                plan_xml = f.read()
            ...
            with open(f"/home/xliq/Documents/LTR_DP/Data/subplans_job_part/{job_nr}/{subquery_nr}/{plan_nr}.txt", "r") as f:
                plan_xml = f.read()
            sql_full = plan_xml.split("OPTION")[0]
            ### parse query
    ...
    class ModelInterface(LossFunction):
        ...
        def fit(self, X_train_vecs, X_train_tree, y_train, X_valid_vecs, X_valid_trees, y_valid,  use_presort=False,
            ...
            for epoch in range(self.epochs):
                ...
                total_curr_job_numbers = get_the_split_of_jobs_list(X_train_vecs, train_ratio=0.3)
