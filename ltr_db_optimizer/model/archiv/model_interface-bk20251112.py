        ...
        def fit(self, X_train_vecs, X_train_tree, y_train, X_valid_vecs, X_valid_trees, y_valid,  use_presort=False,
            ...
            for epoch in range(self.epochs):
                ...
                for batch_i in range(num_batches):
                    ...
                    # print('current sample size: ', sample_size_i)
                    sample_size_i = 10
                    batch_x_p = []
                    batch_x_q = []
                    batch_y = []
                    for idx, x in enumerate(curr_job_numbers):
                        ...
                        if len(curr_keys) < sample_size_i:
                            continue
                        if len(curr_keys) > sample_size_i:
                            curr_keys = random.sample(curr_keys, sample_size_i)
                    ...
                    pred_results = self.net(batch_x_q, batch_x_p, plan_num=sample_size_i)
                ...
                with torch.no_grad():
                    ...
                    y_predicted = self.net.predict_all(valid_batch_x_q, valid_batch_x_p)
                    ...
                    print('validate ndcg', valid_ndcg.shape)
                    valid_ndcg_es.append(valid_ndcg[0])

                    valid_ndcg_new = ndcg_score(valid_batch_y.detach().cpu().numpy(), y_predicted.detach().cpu().numpy(), k=5)
                ...
                avg_valid_loss = round(valid_loss.item()/valid_batch_y.shape[0], 2)
