...
def get_the_split_of_jobs_list(jobs_list, train_ratio):
    '''
...
def create_model(LossFunction, **kwargs):
    class ModelInterface(LossFunction):
        ...
        def __init__(self, epochs=201, sample_size=15, batch_size=100, name="", folder="", **kwargs):
        ...
        def fit(self, X_train_vecs, X_train_tree, y_train, X_valid_vecs, X_valid_trees, y_valid,  use_presort=False,
            ...
            for epoch in range(self.epochs):
                ...
                curr_job_numbers = get_the_split_of_jobs_list(job_numbers, train_ratio=0.3)

                # curr_job_numbers = copy.deepcopy(job_numbers)

                print('curr train jobs and length: ',  len(curr_job_numbers), curr_job_numbers)
                ...
                for idx, x in enumerate(curr_job_numbers):
                    ...
                    curr_y = torch.Tensor([[float(y_train[key]) for key in curr_keys]]).to(self.device)
                    ...
                    loss = self.custom_loss_function(pred_results, curr_y, train=True, label_type=LABEL_TYPE.MultiLabel, presort=use_presort)
                    ...
                    train_ndcg_epoch.append(train_ndcg_value[0][0])

                    train_ndcg_value_new = ndcg_score(curr_y.detach().cpu().numpy(), pred_results.detach().cpu().numpy(), k=5)
                    print('train ndcg new: ', train_ndcg_value_new)
                    train_ndcg_new_epoch.append(train_ndcg_value_new)
                    print('######')


                avg_train_loss_epoch = round(np.mean(train_loss_epoch), 2)
                print(f'Epoch {epoch}: average training loss:', avg_train_loss_epoch)
                all_training_loss.append((epoch, avg_train_loss_epoch))
                avg_train_ndcg_epoch = round(np.mean(train_ndcg_epoch), 2)
                print(f'Epoch {epoch}: average training ndcg:', avg_train_ndcg_epoch)
                all_training_ndcg.append((epoch, avg_train_ndcg_epoch))

                avg_train_ndcg_new_epoch = round(np.mean(train_ndcg_new_epoch), 2)
                ...
                with torch.no_grad():
                    ...
                    ignored = 0
                    print('current valid jobs and length: ', X_valid_vecs.keys(), len(list(X_valid_vecs.keys())))

                    valid_job_numbers = X_valid_vecs.keys()

                    for idx, x_valid in enumerate(valid_job_numbers):
                        curr_plan_keys = X_valid_vecs[x_valid].keys()

                        valid_data_tree = [X_valid_trees[x_valid][key] for key in curr_plan_keys]

                        valid_data_vec = [X_valid_vecs[x_valid][key] for key in curr_plan_keys][0]
                        ...
                        y_predicted = self.net.predict_all(valid_data_vec, valid_data_tree).t()

                        y_true = torch.Tensor([[float(y_valid[key]) for key in curr_plan_keys]]).to(self.device)
                        print('######')
                        print('validate y_predicted', y_predicted[0].detach().cpu().numpy())
                        print('validate y_true: ', y_true[0].detach().cpu().numpy())
                        valid_loss = self.custom_loss_function(y_predicted, y_true, train=False, label_type=LABEL_TYPE.MultiLabel)
                        ...
                        valid_ndcg = ndcg_wrap(y_predicted[0].detach().cpu().numpy(), y_true[0].detach().cpu().numpy())
                        print('validate ndcg', valid_ndcg)
                        valid_ndcg_es.append(valid_ndcg[0][0])

                        valid_ndcg_new = ndcg_score(y_true.detach().cpu().numpy(), y_predicted.detach().cpu().numpy(), k=5)
                        print('validate ndcg new: ', valid_ndcg_new)
                        valid_ndcg_new_list.append(valid_ndcg_new)
                        print('######')

                avg_valid_ndcg = round(np.mean(valid_ndcg_es), 2)
                print(f'Epoch {epoch}: average valid ndcg: ', avg_valid_ndcg)
                avg_valid_loss = round(np.mean(valid_losses), 2)
                print(f'Epoch {epoch}: average valid loss: ', avg_valid_loss)

                avg_valid_ndcg_new = round(np.mean(valid_ndcg_new_list), 2)
