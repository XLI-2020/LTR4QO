    ...
    class ModelInterface(LossFunction):
        ...
        def fit(self, X_train_vecs, X_train_tree, y_train, X_test_vecs, X_test_trees, y_test, use_presort=False,
            ...
            if model_archi_name == "HM":
                self.net = LTRComparisonNet(10, 6).to(self.device)
                print('load LTRComparisonNet HM!!!')
            else:
                self.net = eval(model_archi_name)(10, 6).to(self.device)
            ...
            for epoch in range(self.epochs):
                ...
                with torch.no_grad():
                    ...
                    if epoch % 5 == 0:
                        print(
                            f"Epoch: {epoch} Loss: {curr_loss} Best found Test: {found}/{test_len - ignored} Top k as best: {best_k_found}/{test_len - ignored} Avg. Score Best Test: {avg} Avg k: {avg_k} Worst k: {worst} Avg top k: {avg_best_k_of_k} Worst Best k: {worst_k_best_k}")
