...
class FeatureExtractorGraph:
    ...
    def featurize_node(self, node):
        ...
        # print('current plan node name & id: ', node.name, node.id)
        ...
        if self.with_cost:
            ...
            # print('featurize node: rows, this_vector', rows, this_vector)
            
        # test length of children (2,1,0)
        if node.has_right_child():
            # print('enter featurize left&right childs name, id', node.get_left_child().name, node.get_left_child().id, node.get_right_child().name, node.get_right_child().id)
            ...
            # print('enter featurize left child name, id', node.get_left_child().name, node.get_left_child().id)
            left_child,_ = self.featurize_node(node.get_left_child())
            right_child = (none_child)
        else:
            # print('no childs!', this_vector)
            node.set_featurized_plan((this_vector))
            return (this_vector), node.estimated_rows

        # print('current plan node feature vecs: ', this_vector, left_child, right_child)
...
def  get_features_with_cost_from_folder(plans_folder, cost_folder, return_featurized=True):
    ...
    for file in os.listdir(cost_folder):
        if file.endswith(".txt"):
            ...
            print('type of execution_plan node: ', execution_plan, execution_plan.name, execution_plan.id)
            print("current execution_plan's left_child name & id", execution_plan.left_child.name,
                  execution_plan.left_child.id)
            print("current execution_plan's left_child contained_tables", execution_plan.left_child.contained_tables)
            ...
            if return_featurized:
                ...
                # print('full_execution_plan', full_execution_plan)
                ...
                execution_plan_cp1 = copy.deepcopy(full_execution_plan)

                execution_plan_cp2 = copy.deepcopy(full_execution_plan)


                query_vec = full_execution_plan.get_query_encoding()
                print('RECUR plan file: ', file)
                print('RECUR sql : ', sql)
                print('RECUR type of execution_plan node: ', execution_plan.name, execution_plan.id)
                print("RECUR current execution_plan's left_child name & id", execution_plan.left_child.name, execution_plan.left_child.id)
                # print("current execution_plan's left_child contained_tables",
                #       execution_plan.left_child.contained_tables)

                print('query_encoding: ', np.array(query_vec))


                execution_plan_cp1.calculate_query_encoding()
                recal_query_vec = execution_plan_cp1.get_query_encoding()
                print('recalculated query encoding only top level: ', np.array(recal_query_vec))

                new_node = recalculate_query_encoding(execution_plan_cp2)
...
def featurize_with_labels(plans_folder, cost_folder, label_csv, max_score = 50, score_function = "special", extra_for_min = True, special_border = 0.95):
    ...
    # print('load label data: ', df.head(5))
    # df["Job_nr"] = list(map(lambda x: x.split("_")[0], df.index))
