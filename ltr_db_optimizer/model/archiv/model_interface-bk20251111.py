    ...
    class ModelInterface(LossFunction):
        ...
        def fit(self, X_train_vecs, X_train_tree, y_train, X_valid_vecs, X_valid_trees, y_valid,  use_presort=False,
            ...
            for epoch in range(self.epochs):
                ...
                total_curr_job_numbers = get_the_split_of_jobs_list(job_numbers, train_ratio=1.0)
                ...
                for batch_i in range(num_batches):
                    ...
                    for idx, x in enumerate(curr_job_numbers):
                        ...
                        if len(curr_keys) < self.sample_size:
                            continue
                        if len(curr_keys) > self.sample_size:
                            curr_keys = random.sample(curr_keys, self.sample_size)
                    ...
                    pred_results = self.net(batch_x_q, batch_x_p)
                    ...
                    train_ndcg_value_new = ndcg_score(batch_y.detach().cpu().numpy(), pred_results.detach().cpu().numpy(), k=5)
