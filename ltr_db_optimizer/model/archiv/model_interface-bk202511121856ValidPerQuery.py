...
def create_model(LossFunction, **kwargs):
    class ModelInterface(LossFunction):
        ...
        def __init__(self, epochs=201, sample_size=10, batch_size=64, name="", folder="", **kwargs):
        ...
        def fit(self, X_train_vecs, X_train_tree, y_train, X_valid_vecs, X_valid_trees, y_valid,  use_presort=False,
            ...
            for epoch in range(self.epochs):
                ...
                total_curr_job_numbers = get_the_split_of_jobs_list(job_numbers, train_ratio=0.3)
                ...
                for batch_i in range(num_batches):
                    ...
                    # sample_sizes_combs = [(6, 10), (10, 20), (20, 30), (30, 50)]
                    sample_sizes_combs = [(6, 10), (10, 20), (20, 30), (30, 50)]

                    for sample_size_group in sample_sizes_combs:
                        sample_size_start, sample_size_end = sample_size_group
                        batch_x_p = []
                        batch_x_q = []
                        batch_y = []
                        for idx, x in enumerate(curr_job_numbers):
                            ...
                            if len(curr_keys) < sample_size_start or len(curr_keys) >= sample_size_end:
                                continue
                            curr_keys = random.sample(curr_keys, sample_size_start)
                        ...
                        pred_results = self.net(batch_x_q, batch_x_p, plan_num=sample_size_start)
                        ...
                        print('train ndcg: ', train_ndcg_value.shape)
                        train_ndcg_epoch.append(train_ndcg_value[0])

                        train_ndcg_value_new = ndcg_score(batch_y.detach().cpu().numpy(), pred_results.detach().cpu().numpy(), k=3)
                ...
                with torch.no_grad():
                    ...
                    for idx, x_valid in enumerate(valid_job_numbers):
                        ...
                        y_predicted = self.net.online_predict_all(valid_data_vec, valid_data_tree).t()
                        ...
                        valid_ndcg = ndcg_wrap(y_predicted.detach().cpu(), y_true.detach().cpu())
