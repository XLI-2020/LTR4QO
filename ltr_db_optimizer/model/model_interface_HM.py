...
def create_model(LossFunction, **kwargs):
    class ModelInterface(LossFunction):
        ...
        def __init__(self, epochs=201, sample_size=10, batch_size=64, workload=None, name="", folder="", **kwargs):
        ...
        def fit(self, X_train_vecs, X_train_tree, y_train, X_valid_vecs, X_valid_trees, y_valid,  use_presort=False,
            ...
            input_dim_1 = 10  # 9#10 # 31 lero imdb # 18 lero tpch/stats
            ...
            for epoch in range(self.epochs):
                ...
                # total_curr_job_numbers = get_the_split_of_jobs_list(X_train_vecs, train_ratio=1.0, workload=self.workload)

                total_curr_job_numbers = list(X_train_vecs.keys())
                ...
                # for curr_sample_size in sample_size_list:
                    ...
                    print('y predict: ', pred_results.shape)
                ...
                with torch.no_grad():
                    ...
                    for valid_batch_i in range(num_valid_batches):
                        ...
                        # print('position k y_predicted: ', y_predicted.shape)
                        # print('position k y_true: ', valid_batch_y.shape)
                        position_k_value = 0
                        for i in range(valid_batch_y.shape[0]):
                            temp_position_k = self.position_k.calculate(y_predicted.cpu()[[i], :], valid_batch_y.cpu()[[i], :])
                            # print('temp_position_k: ', temp_position_k)
                            position_k_value += temp_position_k

                        # print('position k ', position_k_value)

                        valid_position_k.append(position_k_value)

                        print('######')
                        print('validate y_predicted', y_predicted.shape)
                        print('validate y_true: ', valid_batch_y.shape)
                ...
                avg_valid_postion_k = round(np.sum(valid_position_k)/np.sum(num_of_actual_valid_samples_epoch), 2)
                ...
                all_valid_position_k.append((epoch, avg_valid_postion_k))
                ...
                if avg_valid_postion_k < best_avg_k:
                    best_avg_k = avg_valid_postion_k
