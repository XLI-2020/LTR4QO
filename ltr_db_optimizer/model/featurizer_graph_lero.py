...
class FeatureExtractorGraph:
    ...
    def __init__(self, with_cost = True, with_issort = False, workload = 'tpch', tree_high=None, tree_low=None, for_lero=True):
        self.vector_length = len(self.type_vector)
        self.with_cost = with_cost
        self.for_lero = for_lero
        if with_cost:
        ...
        self.all_contained_tables_list = []
        if 'tpch' in workload or "Db" in workload:
            ...
            self.table_list = ['lineitem', 'nation', 'orders', 'customer', 'part', 'partsupp', 'region', 'supplier']
            self.table_map_dict = {'lineitem': 'lineitem', 'nation': 'nation', 'orders': 'orders', 'customer': 'customer', 'part': 'part', 'partsupp': 'partsupp', 'region': 'region', 'supplier': 'supplier'}
            ...
            self.tree_low = tree_low
            self.table_list = ['aka_name', 'aka_title', 'cast_info', 'char_name', 'comp_cast_type', 'company_name', 'company_type', 'complete_cast', 'info_type', 'keyword', 'kind_type', 'link_type', 'movie_companies', 'movie_info', 'movie_info_idx', 'movie_keyword', 'movie_link', 'name', 'person_info', 'role_type', 'title']

            # ['t1', 'kt1', 't', 'an', 't2', 'mc', 'n', 'ml', 'cn', 'ct', 'it3', 'mc2', 'rt', 'mc1', 'k', 'mi_idx1', 'mk',
            #  'it1', 'cct2', 'n1', 'it2', 'lt', 'ci', 'cn1', 'kt', 'mi_idx2', 'mi', 'pi', 'cct1', 'chn', 'cc', 'at',
            #  'an1', 'miidx', 'kt2', 'mi_idx', 'it', 'cn2']

            self.table_map_dict = {'mi_idx': 'movie_info_idx' ,  'rt': 'role_type', 'k':'keyword', 'ct':'company_type', 'mk':'movie_keyword', 'mc':'movie_companies', 'at':'aka_title', 'it':'info_type', 'n':'name', 't':'title', 'kt':'kind_type', 'cn':'company_name', 'cct':'comp_cast_type',
             'mi':'movie_info', 'chn':'char_name', 'miidx':'movie_info_idx', 'an':'aka_name', 'lt':'link_type', 'cc':'complete_cast', 'ml':'movie_link', 'ci':'cast_info', 'pi':'person_info'}
            ...
            self.table_list = ['users', 'posts', 'postlinks', 'posthistory', 'comments', 'votes', 'badges', 'tags']
            self.table_map_dict = {'u':'users', 'p':'posts', 'pl':'postlinks', 'ph':'posthistory', 'c':'comments', 'v':'votes', 'b':'badges', 't':'tags'}

        if for_lero:
            self.vector_length += len(self.table_list)
    ...
    def featurize_node(self, node):
        ...
        if self.with_cost:
            # insert cost here.
            this_vector[9] = node.estimated_rows # changed -2
            rows = node.estimated_rows
            # print('curr plan node vec:, this_vector', this_vector)

        # print('nodes contained tables: ', node.contained_tables)
        self.all_contained_tables_list.extend(node.contained_tables)
        # print('all contained tables so far: ', list(set(self.all_contained_tables_list)))

        if self.for_lero:
            for table in node.contained_tables:
                if table in self.table_map_dict:
                    ori_table = self.table_map_dict[table]
                    # print('ori_table: ', table, ori_table)
                    this_vector[10 + self.table_list.index(ori_table)] = 1
                elif table[-1].isdigit():
                    table = table[:-1]
                    if table in self.table_map_dict:
                        ori_table = self.table_map_dict[table]
                        # print('ori_table: ', table, ori_table)
            ...
            # print('this_vector after table id encoding: ', this_vector)
...
def get_features_with_cost_from_folder_Random(plans_folder, cost_folder, workload='tpch', plans_list=None, return_featurized=True, postfix=None):
    ...
    # print('all contained tables:', list(set(feature_ext.all_contained_tables_list)))
    #
